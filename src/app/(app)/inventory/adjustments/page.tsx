...
export const metadata = { title: 'تسويات المخزون' };
...
export default async function Page({
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">تسويات المخزون</h1>
        <p className="mt-1 text-sm text-muted-foreground">زيادة أو عجز يُرحَّل للأستاذ فوراً — كل تسوية تحمل سببها</p>
      </header>

      {canCreate ? (
        <StockOperationForm mode="ADJUSTMENT" />
      ) : (
        <Card>
          <div className="p-6 text-center text-sm text-muted-foreground">
            صلاحيتك تسمح بالاطلاع فقط — التنفيذ يتطلب <span className="bidi-isolate font-mono">inventory.adjustment:create</span>.
          </div>
        </Card>
      )}
        ...
        {totalPages > 1 ? (
          <div className="flex items-center justify-between border-t border-border px-5 py-3 text-sm">
            ...
            <div className="flex gap-2">
              <a
                href={`/inventory/adjustments?page=${Math.max(1, page - 1)}`}
                className={
                  page === 1
              ...
              <a
                href={`/inventory/adjustments?page=${Math.min(totalPages, page + 1)}`}
                className={
                  page === totalPages
                    ? 'pointer-events-none rounded-md border border-border px-3 py-1.5 text-xs opacity-40'
                    : 'rounded-md border border-border px-3 py-1.5 text-xs hover:bg-accent'
